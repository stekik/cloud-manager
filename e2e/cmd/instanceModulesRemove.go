...
var cmdInstanceModulesRemove = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cmdInstanceModulesRemoveOptions.waitDone {
			if cmdInstanceModulesRemoveOptions.moduleName != "cloud-manager" {
			...
			_ = wait.PollUntilContextTimeout(rootCtx, 5*time.Second, cmdInstanceModulesRemoveOptions.timeout, false, func(ctx context.Context) (bool, error) {
				// Check KCP Kyma finalizer
				kcpKyma := &operatorv1beta2.Kyma{}
				err := keb.KcpClient().Get(ctx, types.NamespacedName{
					Namespace: keb.Config().KcpNamespace,
					Name:      cmdInstanceModulesRemoveOptions.runtimeID,
				}, kcpKyma)
				if err != nil && !apierrors.IsNotFound(err) {
					return false, fmt.Errorf("failed to get KCP kyma: %w", err)
				}
				if err == nil && controllerutil.ContainsFinalizer(kcpKyma, api.CommonFinalizerDeletionHook) {
					logger.Info("KCP Kyma deletion-hook finalizer still present")
					return false, nil
				}

				// Check SKR Kyma finalizer
				...
				// Check SKR CloudResources finalizer
			...
			// Force-remove any remaining finalizers so shoot deletion is not blocked.
			// The reconciler gets the grace period above; if it did not act in time, patch them out directly.
			if err := forceRemoveCloudManagerFinalizers(keb, clnt, cmdInstanceModulesRemoveOptions.runtimeID); err != nil {
				return err
			}
...
func forceRemoveCloudManagerFinalizers(keb e2ekeb.Keb, skrClient client.Client, runtimeID string) error {
	// KCP Kyma finalizer
	kcpKyma := &operatorv1beta2.Kyma{}
	...
	if err != nil && !apierrors.IsNotFound(err) {
		return fmt.Errorf("failed to get KCP kyma for force finalizer removal: %w", err)
	}
	if err == nil && controllerutil.ContainsFinalizer(kcpKyma, api.CommonFinalizerDeletionHook) {
		fmt.Printf("Force-removing deletion-hook finalizer from KCP Kyma %s\n", runtimeID)
		base := kcpKyma.DeepCopyObject().(client.Object)
		controllerutil.RemoveFinalizer(kcpKyma, api.CommonFinalizerDeletionHook)
		if err := keb.KcpClient().Patch(rootCtx, kcpKyma, client.MergeFrom(base)); err != nil {
			return fmt.Errorf("failed to force-remove finalizer from KCP Kyma: %w", err)
		}
	}
